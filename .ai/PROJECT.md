...
- **Database**: MongoDB Atlas via **Mongoose**. `lib/mongodb.ts` exports `connectDB()`, which calls `mongoose.connect(MONGODB_URI)` and caches the resulting connection promise on `globalThis` — across HMR reloads in dev and across warm serverless invocations in production — so it doesn't reconnect on every file save or every request. Server-only — import it from API routes/server actions, never from client components. Client code still reaches this data exclusively through the React Query custom-hooks + `lib/api/` pattern above, calling those server routes rather than the database directly.
  - Each collection's schema lives in `lib/models/<collection>.ts` (e.g. `lib/models/ingredient.ts`) as a Mongoose `Schema`/`Model` pair, using the `models.X || model("X", schema)` guard so redefinition doesn't error on Next.js hot reload.
  - Validation (required fields, enums, etc.) lives on the Mongoose schema itself, not a separate `$jsonSchema` validator. Uniqueness and other indexes are declared with `schema.index(...)` on the model; seed/setup scripts under `scripts/` call `Model.init()` to ensure those indexes are built before writing (see `scripts/seed-ingredients.mjs`). Run one-off scripts with `node --env-file=.env.local scripts/<name>.mjs` — no `dotenv` dependency needed (Node 20.6+ reads `--env-file` natively). Seed scripts upsert by a natural unique key so they're safe to re-run.
