...
ingredient"/Save in that popup — **not** a special double-click, just the
normal single click that finishes the popup — results in **two duplicate
recipes** being saved, not a duplicate ingredient. (An earlier pass at this
entry incorrectly diagnosed the symptom as a duplicated *ingredient* row
and chased a double-submit *race*, based on the report alone without a
live repro; live-testing that race at the API level came back clean —
see below — and the actual report, confirmed by the reporting dev, was
recipe duplication. Leaving that section in for the record since the fix
that resolved it touches the same code.)
**Cause (best-supported theory — not re-derived from a live browser trace,
but consistent with the confirmed fix)**: `IngredientFormDialog`'s popup
content renders via `DialogPortal` (Base UI → `ReactDOM.createPortal`),
so its `<form onSubmit={handleFormSubmit}>` is physically attached to
`document.body`, *outside* `RecipeForm`'s own `<form onSubmit={handleSubmit}>`
in the DOM. But it is still a descendant of `RecipeForm` in the **React
component tree** (`RecipeForm` → `IngredientCombobox` → `IngredientFormDialog`),
and React's documented portal behavior is that a fired event "propagate[s]
to ancestors in the containing React tree, even if those elements are not
ancestors in the DOM tree." `handleFormSubmit` called `event.preventDefault()`
but never `event.stopPropagation()`, so the "submit" event could continue
bubbling — through the React tree, not the DOM tree — up to `RecipeForm`'s
own `<form onSubmit={handleSubmit}>`, triggering a recipe save at the exact
moment the ingredient popup was submitted (likely incomplete/premature,
since ingredient creation is async and the new row isn't in `rows` yet).
The user's later, real "Save Recipe" click then saves a second, complete
recipe — two recipes saved from one recipe-creation session.
**Solution (confirmed working by the reporting dev)**: The synchronous
double-submit guard added to both dialogs — a `useRef` (not `useState`;
state updates aren't visible until the next render, so they can't close a
same-tick re-entrant call) set `true` the instant a submit starts, checked
at the top of the handler, and cleared via `useEffect` once the mutation
settles (success *or* error) — is what's currently in the codebase and is
confirmed to have resolved this:
...
(`handleSubmit`). **Refs must only be written inside effects or event
handlers, never during render** — the existing "adjust state during
render" pattern in this codebase (e.g. `ingredient-form-dialog.tsx`'s
`wasOpen` reset block) is specifically sanctioned for *state*; doing the
same to a ref trips the `react-hooks/refs` ESLint rule and is unsafe under
React's rendering model (a render can be discarded/retried). Use a
`useEffect` keyed on the value you're reacting to instead.
**A more surgical fix at the actual source, not yet applied**: adding
`event.stopPropagation()` alongside `event.preventDefault()` in
`ingredient-form-dialog.tsx`'s `handleFormSubmit`/`handleConfirmEdit` would
stop the submit event from ever reaching `RecipeForm`'s outer form in the
first place, addressing the cause directly rather than only guarding
against its symptom. Worth doing if this (or a variant of it — e.g. a
future dialog nested inside a different outer form) resurfaces; not applied
here since the ref guard already resolved the reported behavior.
**Live-tested against the real Atlas cluster (confirms a different layer is
solid, not the cause of this bug)**: fired two genuinely concurrent
...
back clean. This rules out "two Ingredient documents with the same name"
as a live concern, independent of the recipe-duplication bug above.
